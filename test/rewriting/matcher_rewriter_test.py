...
def rule_matches(diagram: Diagram, rule: RewriteRule) -> bool:
    matcher = Matcher(diagram)
    match = matcher.match_rule(rule, apply=False)
    return match is not None

def rewrite(diagram: Diagram, rule: RewriteRule):
    matcher = Matcher(diagram)
    match = matcher.match_rule(rule, apply=True)
